...
term_signal_handler() {
    log_error "Signal Interrupt! TERM"
    exit 4
}
...
display_usage() {
    cat <<EOF
udm-tools hook template

Usage: ${SCRIPT} [OPTION] start|stop hook-type

Commands:
  start               Starts this hook
  stop                Stops this hook
...
__hook_start() {
    log_info "Running hook start - $SCRIPT"
}

__hook_stop() {
    log_info "Running hook stop - $SCRIPT"
}

__hook_restart() {
    log_info "Running hook restart - $SCRIPT"
}
...
main() {
    ...
    # checks if both invoke action and hook-type are specified
    if [ "$#" -ne 2 ]; then
        log_error "Invalid number of arguments($#) expects 2"
        display_usage
        exit 1
    fi
    ...
    restart)
        ...
        ;;
    *)
        log_error "Invalid invoke_mode - ${1}. See usage below."
        exit 1
        ;;
    esac
